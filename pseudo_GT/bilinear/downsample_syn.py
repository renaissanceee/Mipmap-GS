...
def main(source, gt_root, scenes):
    for scene in scenes:
        ...
        for scale in [8, 4, 2]:
            print("scale:", scale)
            output_dir = os.path.join(output_root_dir, f"resize_x{scale}", "test")
            os.makedirs(output_dir, exist_ok=True)
            # copy json
            command = f"cp {source}/{scene}/transforms_test.json {output_root_dir}/resize_x{scale}/"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gt_root', type=str, default="benchmark_nerf_synthetic_stmt_down",help="imgs")
