...
def train_scene(gpu, scene, factor):
    cmd = f"python train.py -s {dataset_dir}/{scene} -m {output_dir}/{scene} --eval -r {factor} --port {6009 + int(gpu)}"
    print(cmd)
    if not dry_run:
        os.system(cmd)

    for scale in [8, 4, 2, 1]:
        cmd = f"python render.py -m {output_dir}/{scene} -r {scale} --scale {scale} --data_device cpu --skip_train"
        print(cmd)
        if not dry_run:
            os.system(cmd)
