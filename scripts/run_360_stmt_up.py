...
output_dir = "benchmark_360v2_stmt_up"
dataset_dir = "../3D-Gaussian/360_v2"
...
def train_scene(gpu, scene, factor):
    cmd = f"python train.py -s {dataset_dir}/{scene} -m {output_dir}/{scene} --eval -r {factor} --port {6001 + int(gpu)}"
    print(cmd)
    if not dry_run:
        os.system(cmd)

    for scale in [8, 4, 2, 1]:
        cmd = f"python render.py -m {output_dir}/{scene} -r {scale} --data_device cpu --skip_train --scale {scale}"
        print(cmd)
        if not dry_run:
            os.system(cmd)
