...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from,load_gaussian):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        viewpoint_cam.image_height, viewpoint_cam.image_width = H, W # 1644, 2472
        ...
        target_size = (gt_image.shape[1], gt_image.shape[2])
        gt_image = gt_image.cuda()
        image = F.interpolate(image.unsqueeze(0), size=target_size, mode='bilinear',align_corners=False,antialias=True).squeeze(0)
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:  # 15_000
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:  # iter>500, every 100 iters
                    size_threshold = 20 if iteration > opt.opacity_reset_interval else None# None until 3000 iter

                    gaussians.densify_and_prune(opt.densify_grad_threshold, 0.005, scene.cameras_extent, size_threshold)

                if iteration % opt.opacity_reset_interval == 0 or (
                        dataset.white_background and iteration == opt.densify_from_iter+1):
...
if __name__ == "__main__":
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations,args.checkpoint_iterations,
             args.start_checkpoint, args.debug_from, args.load_gaussian)
    # All done
    print("\nTraining complete.")
